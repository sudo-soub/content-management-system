...
class BlogsGetPostView(views.APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        blogs = Blogs.objects.values(
            "id", "blogname", "blog_data", "user__username", "user__first_name",
            "user__last_name"
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        username = request.data.pop("user")
        user = User.objects.get(username=username).pk
        request.data["user"] = user
...
class BlogsRetrieveUpdateView(ModelViewSet):
    ...
    queryset = Blogs.objects.values(
        "blog_data", "blogname", "user__username", "user__first_name",
        "user__last_name"
    )
...
class GetUpdateBlogsByUser(views.APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        user = self.request.user
        queryset = Blogs.objects.filter(user__username='dipun1').values(
            "blog_data", "blogname", "user__username", "user__first_name",
            "user__last_name", "id"
        )
